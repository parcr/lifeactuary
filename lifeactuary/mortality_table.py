...
class MortalityTable:
    '''
    Instantiates a life table, where you can pass it in the form of the lx, qx or px. Note that the first value corresponds
    to the first age considered in the table.
...
### Survival Probabilities Functions
    ...
    def npx(self, x, n=1, method='udd'):
        '''
        Obtains the probability that a life x reaches age x+t

        :param method: the method used to approximate lx for non-integer x's
        :param x: age at beginning
        :param n: period
        :return: probability of x surviving beyond age x+t
        '''
        if method not in self.__methods:
            return np.nan
        if x < 0:
    ...
    def get_integral_px_method(self, x, method='udd'):
        '''
        This function can be used to approximate the life expectancy between to ages, using different interpolation rules.

        :param x: the value of x, that should be integer
        :method: the chosen method to approximate px: 'udd', 'cfm' or 'bal'
    ...
    def exn(self, x, n, method='udd'):
        ...
        :param method: The method used to interpolate: 'udd', 'cfm' or 'bal'.
        :return: The approximated life span between x and x+n, considering the interpolation methods implemented
        to approximate the integral of the survival function.
        '''
        if method not in self.__methods:
            return np.nan
        if x < 0:
